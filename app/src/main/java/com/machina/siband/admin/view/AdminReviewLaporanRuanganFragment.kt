...
class AdminReviewLaporanRuanganFragment : Fragment() {
  ...
  override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
    ...
    (binding.fragmentAdminReviewLaporanStatus.editText as? AutoCompleteTextView)?.setAdapter(
      mArrayAdapter
    )
    ...
    Intent(MediaStore.ACTION_IMAGE_CAPTURE).also { takePictureIntent ->
      ...
      takePictureIntent.resolveActivity(packageManager)?.also { component ->
        ...
        photoFile?.also {
          ...
          startActivityForResult(
            takePictureIntent, CAPTURE_IMAGE_CODE
          )
        }
      }
    }
  ...
  private fun resolveForm() {
    ...
    if (dok > 0) {
      ...
      repeat(dok) {
        val storageRef =
          FirebaseStorageRepo.getLaporanImageRef(email, tanggal, lokasi, "${nama}$it")
        loadImageInternet(
          mLayoutParams,
          binding.fragmentAdminReviewLaporanDokumentasiContainer,
          storageRef
        )
      }
    }
    ...
    if (dokPerbaikan > 0) {
      ...
      repeat(dokPerbaikan) {
        val storageRef =
          FirebaseStorageRepo.getLaporanPerbaikanImageRef(email, tanggal, lokasi, "${nama}$it")
        loadImageInternet(
          mLayoutParams,
          binding.fragmentAdminReviewLaporanDokumentasiPerbaikanContainer,
          storageRef
        )
      }
    }
