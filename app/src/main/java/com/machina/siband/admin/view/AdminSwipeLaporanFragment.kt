...
class AdminSwipeLaporanFragment : Fragment() {
  ...
  private fun onDialogDateSet(calendar: Calendar) {
    val tanggal = "${calendar.get(Calendar.DAY_OF_MONTH)}-${calendar.get(Calendar.MONTH) + 1}-${
    ...
    for (lantai in allLaporanByLantai) {
      for (laporanArea in lantai) {
        for (laporan in laporanArea) {
          Log.d(
            TAG,
            "index [${laporan.index}] lantai [${laporan.lantai}] area [${laporan.area}] ruangan [${laporan.lokasi}] nama [${laporan.nama}]"
          )
        }
      }
    }
...
//                        Log.d(TAG, "lantai [${laporan.lantai}] area [${laporan.area}] ruangan [${laporan.lokasi}] item [${laporan.nama}]")
      ...
      brokenLaporan.forEachIndexed { index, item ->
        val tempInternal = MutableList(10) { "" }
        ...
        tempInternal[5] = item.status
        tempInternal[8] = item.keterangan
        tempInternal[9] = item.pelapor
        ...
        if (dok > 0) {
          var dokText = ""
          repeat(dok) {
            val ref = FirebaseStorageRepo.getLaporanImageRef(
              item.email,
              tanggal,
              item.lokasi,
              "${item.nama}$it"
        ...
        if (dokPerbaikan > 0) {
          var dokText = ""
          repeat(dok) {
            val ref = FirebaseStorageRepo.getLaporanPerbaikanImageRef(
              item.email,
              tanggal,
              item.lokasi,
              "${item.nama}$it"
